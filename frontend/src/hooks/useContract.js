...
function readActivityLog() {
    try {
        return JSON.parse(localStorage.getItem("otc-tx-log") ?? "{}");
    }
    catch {
        return {};
    }
...
export function useContract(walletAddress) {
    ...
    const contract = useMemo(() => {
        ...
        return window.tronWeb.contract(abi, OTC_SETTLEMENT_ADDRESS);
    }, [walletAddress]);
    async function refreshDeal(dealId) {
        if (!contract) {
            return null;
        }
    ...
    async function waitForDealUpdate(dealId, predicate, timeoutMs = 20000) {
    ...
    async function waitForTransactionSuccess(txHash, context, timeoutMs = 45000) {
        const startedAt = Date.now();
        while (Date.now() - startedAt < timeoutMs) {
    ...
    async function loadMyDeals() {
        ...
        const ids = Array.from(idsRaw).map((value) => Number(String(value)));
    ...
    async function approveIfNeeded(tokenAddress, amount, action, dealId) {
        const token = getTokenByAddress(tokenAddress);
        if (!token || token.kind !== "trc20") {
            return;
        }
        const tokenContract = await window.tronWeb.contract(trc20Abi, tokenAddress);
    ...
    async function runAction(context, fn) {
        setLoading(true);
        setError(null);
        try {
            return await fn();
    ...
    async function createDeal(input) {
        ...
        return runAction("deal creation", async () => {
            ...
            const txHash = await contract.createDeal(tokenA.kind === "native" ? NATIVE_TRX_ADDRESS : tokenA.address, toUnits(input.amountA, tokenA).toString(), tokenB.kind === "native" ? NATIVE_TRX_ADDRESS : tokenB.address, toUnits(input.amountB, tokenB).toString(), input.counterparty, input.timeoutHours).send({ feeLimit: DEFAULT_FEE_LIMIT });
    ...
    async function fundDeal(deal) {
        ...
        return runAction("deal funding", async () => {
            ...
            const token = getTokenByAddress(tokenAddress) ?? {
                ...
                kind: tokenAddress === NATIVE_TRX_ADDRESS ? "native" : "trc20"
            };
            if (token.kind === "trc20") {
    ...
    async function confirmSettlement(deal) {
        if (!contract) {
            throw new Error("Contract unavailable");
        }
        return runAction("settlement confirmation", async () => {
            ...
            try {
                ...
                const updated = await waitForDealUpdate(deal.id, (candidate) => isPartyA ? candidate.partyAConfirmed : candidate.partyBConfirmed, 45000).catch(() => null);
                if (!updated) {
    ...
    async function executeSettlement(deal) {
        if (!contract) {
            throw new Error("Contract unavailable");
        }
        return runAction("final settlement", async () => {
            ...
            try {
                ...
                const updated = await waitForDealUpdate(deal.id, (candidate) => candidate.status === "Settled", 45000).catch(() => null);
                if (!updated) {
    ...
    async function raiseDispute(dealId, reason) {
        if (!contract) {
            throw new Error("Contract unavailable");
        }
    ...
    async function resolveDispute(dealId, refundBoth) {
        if (!contract) {
            throw new Error("Contract unavailable");
        }
    ...
    async function claimExpired(dealId) {
        if (!contract) {
            throw new Error("Contract unavailable");
        }
